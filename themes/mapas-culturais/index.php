...
<div class="row mt-20 pt-60">
    <?php if(is_author()): $title = 'Conteúdo de '.$curauth->display_name ?>
        <div class="column large-12 small-12 text-center author--info">
            ...
            <div>
                <h2 class="author--info-name fz-30 ls-4"><strong><?php echo  $curauth->display_name ?></strong> </h2>
                <div class="author--info-biography"><?php echo  nl2br(get_the_author_meta('description')) ?></div>
            </div>
        </div>
    <?php elseif(is_tag() || is_category() || is_tax() || is_archive() ): ?>
        <div class="column large-12 small-12 text-center">
            <div class="title-button text-left mb-20">
                <h3> <?php echo is_tag() ? "Tag:" : (is_category() || is_tax() ? "Categoria:" : post_type_archive_title( '', false ) ) ?> <strong><?php echo  single_cat_title( '', false ) ?></strong>  </h3>
            </div>
        </div>
